...
void log_msg(const char *format, ...)
{
    ...
    vfprintf(HB_DATA->logfile, format, ap);
}
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct hb_state *)context->private_data), logfile, %08x, );
    log_struct(((struct hb_state *)context->private_data), dir_ssd, %s, );
    log_struct(((struct hb_state *)context->private_data), dir_hdd, %s, );
