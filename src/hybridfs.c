    ...
    if ( (dir = opendir(fpath_for_ssd)) != NULL ) {
        while ( (ent = readdir(dir)) != NULL ) {
            ...
            if (st.st_size >= HB_DATA->mig_threshold) {
                ...
                rename(fpath_file, target_path);    //// CS but lock is done in hb_open and hb_release
...
int hb_open(const char *path, struct fuse_file_info *fi)
{
    ...
    if (fd >= 0) {
        //// Initiate files' mutex
        if (!hb_findMutex(path)) {
            //printf("[hb_open] creation of path : %s\n",path);
            ...
            /*printf("[hb_open read CS with create] %s\n", path);
	     ...
	     struct fuse_file_info *fi)
{
    ...
    */
    ...
    ret = log_syscall("pwrite", pwrite(fi->fh, buf, size, offset), 0);  //// CS
	       ...
	       struct fuse_file_info *fi)
{
    ...
    do {
        ...
        if (filler(buf, de->d_name, NULL, 0) != 0) {
            ////log_msg("    ERROR hb_readdir filler:  buffer full");
            return -ENOMEM;
        }
